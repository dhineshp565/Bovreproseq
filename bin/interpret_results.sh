...
echo -e "SAMPLEID,PATHOGEN,RESULT,STATUS" > "$1_results.csv"

# loop through the list of target names
while read lines; do
	target=$(echo "${lines}" | cut -d ',' -f 1) # extract the target name
    amplicon=$(cut -f 15 "$1"| grep -F "${target}") # check for the presence of target name in the 15th column of the abricate output
    if [ "${amplicon}" ]; then
